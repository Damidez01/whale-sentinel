...
const { isFlagged, shortAddr } = require('../intelligence/flagged');
...
function parseAsset(raw) {
  // "ETH.ETH" -> "ETH", "ETH.USDC-0xa0b..." -> "USDC", "BTC.BTC" -> "BTC"
  if (!raw) return null;
...
function getDirection(inAsset, outAsset) {
  const inIsETH  = ETH_ASSETS.has(inAsset);
  const outIsBTC = outAsset === 'BTC';
  const inIsBTC  = inAsset === 'BTC';
  const outIsETH = ETH_ASSETS.has(outAsset);

  if (inIsETH && outIsBTC) return 'ETH_TO_BTC';
  if (inIsBTC && outIsETH) return 'BTC_TO_ETH';
  return null;
}

function directionLabel(direction) {
  if (direction === 'ETH_TO_BTC') return { emoji: '🔴', label: 'Exit to BTC' };
  if (direction === 'BTC_TO_ETH') return { emoji: '🟡', label: 'BTC entering ETH ecosystem' };
...
async function processSwap(swap) {
  try {
    ...
    // Only care about ETH<->BTC directions
    if (!direction) return;
    ...
    const walletKey = `thor:wallet:${fromAddr}`;

    // ── Burst detection: same wallet, multiple large swaps ──
    const burstCount = windowAdd(walletKey, usdValue, BURST_WIN_MIN * 60);

    if (burstCount >= BURST_COUNT) {
      const allSwaps  = windowGet(walletKey, BURST_WIN_MIN * 60);
      const totalUSD  = allSwaps.reduce((s, v) => s + Number(v), 0);

      sendAlert({
        chain: 'THOR',
        title: `🚨 CRITICAL — THORChain Burst ${label}`,
        alertId: `thor:burst:${fromAddr}:${burstCount}`,
        body: [
          `Direction: ${emoji} ${inAsset} → ${outAsset}`,
          `Wallet: \`${shortAddr(fromAddr)}\``,
          ``,
          ...
          ``,
          `🔗 [THORChain Explorer](https://thorchain.net/tx/${txId})`,
        ].join('\n'),
      });
      return;
    }
    ...
    logger.alert(`[THOR] ${direction} | ${inAmount.toFixed(2)} ${inAsset} → ${outAmount.toFixed(4)} ${outAsset} | ${fmtUSD(usdValue)}`);
...
function startTHORChainMonitor() {
  logger.info('[THOR] THORChain monitor starting — polling Midgard every 15s');
  logger.info(`[THOR] Watching: ETH/stables ↔ BTC | Min: ${fmtUSD(MIN_USD)}`);
  poll();
